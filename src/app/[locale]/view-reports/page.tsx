...
export async function generateMetadata({ params }: ViewReportsPageProps) {
  const resolvedParams = await params;
  return await generateDynamicPageMetadata('/view-reports', resolvedParams.locale);
}

export default async function ViewReportsPage({ params }: ViewReportsPageProps) {
  try {
    const resolvedParams = await params;
    const data = await getDynamicPageData('/view-reports', resolvedParams.locale);
    ...
    console.error('Error rendering view-reports page:', error);
    notFound();
  }
}