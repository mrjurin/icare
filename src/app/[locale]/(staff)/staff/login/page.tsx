...
import { Eye, EyeOff, AlertCircle, Loader2, Mail, Lock, CheckCircle2, UserCheck, ArrowLeft, Home } from "lucide-react";
...
export default function StaffLoginPage() {
  ...
  const validateLoginId = (value: string) => {
    if (!value.trim()) {
      setLoginIdError(t("emailOrIcRequired"));
      return false;
    }
    ...
    if (!isEmail && !isIc) {
      setLoginIdError(t("emailOrIcInvalid"));
      return false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEmail) {
        ...
        if (staffError || !data) {
          setError(t("errors.invalidIcOrPassword"));
          setLoading(false);
          return;
        }
            ...
            >
              ...
              <div className="absolute bottom-8 left-8 right-8 text-white">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h2 className="text-2xl font-bold">{t("staffPortal")}</h2>
                </div>
                <p className="text-sm text-gray-200">{t("portalDescription")}</p>
              </div>
            </div>
          </div>
              ...
              <div className="flex flex-col gap-5">
                ...
                <label className="flex flex-col gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[#111418] dark:text-gray-300 text-sm font-semibold">{t("emailOrIcNumber")}</span>
                  </div>
                  <Input
                    type="text"
                    placeholder={t("emailOrIcPlaceholder")}
