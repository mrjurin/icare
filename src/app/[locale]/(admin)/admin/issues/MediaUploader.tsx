...
type Props = {
  onMediaChange?: (media: Array<{ url: string; type?: string; size_bytes?: number }>) => void;
};

export default function MediaUploader({ onMediaChange }: Props) {
  ...
  useEffect(() => {
    if (onMediaChange) {
      onMediaChange(uploaded.map(({ preview, ...rest }) => rest));
    }
  ...
  const uploadFiles = async (files: File[]) => {
    ...
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) {
      setError(t("authRequired") || "You must be logged in to upload files. Please log in and try again.");
      return;
    }
    ...
    try {
      for (const file of files) {
        ...
        if (uploadError) {
          ...
          if (uploadError.message.includes("row-level security policy")) {
            errorMessage = t("permissionDenied") || "Permission denied. Please ensure you are logged in and have permission to upload files.";
          } else if (uploadError.message.includes("new row violates")) {
            errorMessage = t("storagePermissionDenied") || "Storage permission denied. Please check your account permissions.";
          }
          setError(`${t("failedToUpload") || "Failed to upload"} ${file.name}: ${errorMessage}`);
          continue;
        }
      ...
      setError(err instanceof Error ? err.message : t("uploadError") || "An error occurred during upload");
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="flex flex-col items-center gap-3">
          ...
          <div>
            <p className="text-sm text-gray-700 dark:text-gray-300">
              <span className="text-primary font-semibold">{t("uploadFiles") || "Upload files"}</span> {t("orDragAndDrop") || "or drag and drop"}
            </p>
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {t("supportsMultipleMedia") || "Supports multiple images & videos (up to 25MB total)"}
            </p>
            <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">
              {t("used") || "Used"}: {formatFileSize(totalSize)} / {formatFileSize(MAX_TOTAL_SIZE)}
            </p>
            {isAuthenticated === false && (
              <p className="mt-2 text-xs text-orange-600 dark:text-orange-400 font-medium">
                ⚠️ {t("mustBeLoggedIn") || "You must be logged in to upload files"}
              </p>
            )}
          </div>
        ...
        {error && (
          <div className="mt-4 flex items-start gap-2 rounded-lg bg-red-50 dark:bg-red-900/30 p-3 text-sm text-red-700 dark:text-red-400">
            ...
            {uploaded.map((item, index) => (
              ...
              >
                ...
                <button
                  ...
                  className="absolute top-1 right-1 p-1.5 rounded-full bg-red-500 text-white opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity hover:bg-red-600 touch-manipulation"
                  type="button"
