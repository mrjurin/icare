...
import {
  startLocalityGeocodingJob,
  getLatestLocalityGeocodingJob,
  pauseLocalityGeocodingJob,
  resumeLocalityGeocodingJob,
} from "@/lib/actions/localities";

export default function LocalityGeocodingSection() {
  const t = useTranslations("referenceData.geocoding");
  ...
  useEffect(() => {
    ...
    const checkJobStatus = async () => {
      const result = await getLatestLocalityGeocodingJob();
  ...
  const handleGeocode = async () => {
    ...
    try {
      const result = await startLocalityGeocodingJob();
      if (!result.success) {
      ...
      if (result.data) {
        ...
        const jobResult = await getLatestLocalityGeocodingJob();
  ...
  const handlePause = async () => {
    ...
    setGeocodeError(null);
    try {
      const result = await pauseLocalityGeocodingJob(geocodeJob.id);
      if (!result.success) {
  ...
  const handleResume = async () => {
    ...
    setGeocodeError(null);
    try {
      const result = await resumeLocalityGeocodingJob(geocodeJob.id);
      if (!result.success) {
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-900 dark:text-white">
                  {geocodeJob.status === "paused"
                    ? t("paused", {
                        processed: geocodeJob.processedLocalities,
                        total: geocodeJob.totalLocalities,
                      })
                  ...
                  <div className="mt-3">
                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${Math.min(
                            (geocodeJob.processedLocalities / geocodeJob.totalLocalities) * 100,
                            100
                          )}%`,
                        }}
                      />
                    ...
                    <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                      {Math.round((geocodeJob.processedLocalities / geocodeJob.totalLocalities) * 100)}%{" "}
                      {t("complete")}
                    </p>
                  </div>
                )}
