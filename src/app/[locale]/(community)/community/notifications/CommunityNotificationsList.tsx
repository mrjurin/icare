...
import { 
  markAllNotificationsAsReadForProfile, 
  markNotificationAsReadForProfile, 
  deleteNotificationForProfile 
...
type CommunityNotificationsListProps = {
...
export default function CommunityNotificationsList({ 
  notifications: initialNotifications,
  locale 
}: CommunityNotificationsListProps) {
  ...
  const handleMarkAllAsRead = async () => {
    ...
    const result = await markAllNotificationsAsReadForProfile();
    if (result.success) {
  ...
  const handleMarkAsRead = async (id: number) => {
    ...
    const result = await markNotificationAsReadForProfile(id);
    if (result.success) {
      setNotifications((prev) =>
  ...
  const handleDelete = async (id: number) => {
    ...
    setProcessingIds((prev) => new Set(prev).add(id));
    const result = await deleteNotificationForProfile(id);
    if (result.success) {
          ...
          filteredNotifications.map((n) => (
            <div key={n.id} className={`rounded-xl border border-gray-200 bg-white px-4 py-4 flex items-center justify-between dark:bg-gray-800 dark:border-gray-700 ${n.unread ? "outline outline-2 outline-primary/60" : ""}`}>
              ...
              <div className="flex items-center gap-3">
                {n.issueRef && (
                  <Link 
                    href={`/${locale}/community/issues/${n.issueRef.replace(/^INC-/, "")}`} 
