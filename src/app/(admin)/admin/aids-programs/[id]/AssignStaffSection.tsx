...
export default function AssignStaffSection({ program, assignedStaffIds: initialAssignedStaffIds }: AssignStaffSectionProps) {
  ...
  const handleSave = async () => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        setSuccessMessage("Staff assignments saved successfully!");
        router.refresh();
  ...
  return (
    <div className="bg-white dark:bg-background-dark rounded-lg border border-gray-200 dark:border-gray-700 p-6">
      <h2 className="text-xl font-bold mb-4">Assign Staff Members</h2>
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
        Select staff members to handle this aids program. They will be able to mark households as receiving aids in their assigned zones/villages.
      </p>
      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg p-3">
            {staffList.length === 0 ? (
              <p className="text-xs text-gray-500 dark:text-gray-400">No active staff available</p>
            ) : (
              staffList.map((staff) => (
                <label
                  key={staff.id}
